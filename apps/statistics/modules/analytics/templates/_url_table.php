  ...
  <tbody>
    <?php foreach($pData['data'] as $i => $data): ?>
      <tr class="<?php echo $i%2==0 ? 'odd' : 'even' ?>">
      	<td><?php echo ++$i;?>.</td>
        <td><?php echo link_to($data['url'], '@get_analytics_url_details', array('query_string' => 'com=all&url='.$data['url'].'&host_id='.$pHostId.'&date-from='.$pFrom.'&date-to='.$pTo, 'class' => 'analytix-filter-link'));  ?></td>
