...
<?php if($pDeal->getState() == 'paused') { ?>
			<td align="center" class="first"><div><?php echo image_tag('/img/global/24x24/clock.png', array('title' => __('Deal stopped')))?></div></td>
		  <td align="left">
		  	<div class="marketbox" title="<?php echo $pDeal->getSummary(); ?>">
					<?php if(strtotime($pDeal->getStartDate()) > time()) {?>
						<?php echo $pDeal->getSummary(); ?>
					<?php } else { ?>
			  		<?php echo link_to(truncate_text($pDeal->getSummary(), 20), 'deal_analytics/deal_statistics?deal_id='.$pDeal->getId(), array('class' => 'paused-link')); ?>
			  	<?php } ?>
		  	</div>
		  </td>
		  <td align="left" valign="middle"><div class="marketbox"><?php echo $pDeal->getDomainProfile()->getUrl(); ?></div></td>
      	...
      	<?php if($pDeal->getCouponType() == "single"){ ?>data-obj='{"action":"/deals/save_quantity", "type":"text",  "params": "{\"deal_id\":\"<?php echo $pDeal->getId(); ?>\", \"cssid\":\"deal-quantity-<?php echo $pDeal->getId(); ?>\"}"}'
      	<?php } else { ?> data-obj='{"action":"/deals/paste_codes?deal_id=<?php echo $pDeal->getId(); ?>", "type":"layer", "params": "{\"deal_id\":\"<?php echo $pDeal->getId(); ?>\", \"cssid\":\"deal-quantity-<?php echo $pDeal->getId(); ?>\"}"}'
      	<?php } ?>><div><?php echo __($pDeal->getHumanCouponQuantity()); ?></div></td>
		  <td align="center"><div><?php echo link_to(image_tag('/img/global/24x24/tools.png', array('title' => __('Edit Deal'))), '/deals/get_create_form?deal_id='.$pDeal->getId(), array('class' => 'link-deal-content', "title" => __('Go to analytics for %deal%', array('%deal%' => $pDeal->getSummary())))); ?></div></td>
		  <td align="center" class="last">
      	<?php echo link_to(image_tag('/img/global/24x24/trash.png', array('title' => __('Delete Deal'))), '/deals/set_state?deal_id='.$pDeal->getId().'&state=trashed', array('class' => 'edit-state')); ?>
